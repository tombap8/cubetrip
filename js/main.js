...
/////// 위치값 변수 셋팅 ///////////////////
// 큐브를 회전하기 위한 transform 속성셋팅값 //
// 다채로운 회전을 위해 기존값에 360도를 더해준다
// 예) rotateX(-90deg) -> rotateX(-450deg)
// 예) rotateX(0deg) 
//   -> rotateX(360deg) / rotateX(-360deg)
// 그리고 360도를 줄때 한번은 반대방향으로 설정해본다!
// 단, "출발"은 처음값 그대로~!
var pos = {
    "출발": "translate(-50%, -50%) rotateX(0deg) rotateY(0deg)", //0,0
    "서울": "translate(-50%, -50%) rotateX(-450deg) rotateY(360deg)", //-450,360
    "뉴욕": "translate(-50%, -50%) rotateX(450deg) rotateY(-360deg)", //450,-360
    "파리": "translate(-50%, -50%) rotateX(360deg) rotateY(540deg)",
    //360,540
    "베를린": "translate(-50%, -50%) rotateX(720deg) rotateY(450deg)",
    //720,450
    "런던": "translate(-50%, -50%) rotateX(-720deg) rotateY(-450deg)" //-720,-450
}; ///// pos객체 //////////////////
...
$(function () { /////// jQB ///////////////////////
    ...
    // 1. 큐브 회전시키기
    // - 방법: 메뉴를 클릭하면
    // 미리셋팅된 transform 정보를 큐브박스에 css로 넣는다!
    // 이벤트 대상: .city a
    // 변경 대상: .cube

    // 광클금지
    var prot = 0; //0-허용,1-불허용
    // 현재메뉴
    var cmenu = "출발";
    ...
    $(".city a").click(function () {
        ...
        ////////// 광클금지 /////////
        // 잠금설정값이 1 이거나 현재클릭된 메뉴과 같으면 돌아가!
        if (prot === 1 || cmenu === txt) return false;
        prot = 1; //잠금
        setTimeout(function () {
            prot = 0; //해제
        }, 1800);
        ...
        if (txt !== "출발") {
            ...
            // 도시명 넣기(.cbox h2)
            cbox.find("h2").text(txt);

            // 도시정보 넣기(.cbox p)
            cbox.find("p").text(city)

            // 도시정보 박스(.cbox)를 보이게함
            // 큐브회전시간(1.5초)후에 등장함
            cbox.delay(1500).fadeTo(300, 1);
